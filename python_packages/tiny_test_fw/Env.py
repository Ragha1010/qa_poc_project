...
class Env(object):
    ...
    @_synced
    def get_dut(self, dut_name, app_path, context, dut_class=None, app_class=None, app_config_name=None, **dut_init_args):
        """
        ...
        :param context: the context of behave
        ...
        if dut_name in self.allocated_duts:
            ...
            try:
                ...
                available_ports, avail_port_info = dut_class.list_available_ports()
            ...
            if dut_name is None:
                ...
                app_inst = app_class(app_path, app_config_name, app_target, context)
            ...
            if port:
                ...
                if dut_name == 'init':
                    vid = None
                    for x in avail_port_info:
                        if x['port'] == port:
                            vid = x['vid']
                            break

                    if vid == context.hub_vid:
                        dut_class = ESYSHubDut
                        dut_name = 'Hub'
                        app_path = 'si_app'
                    elif vid == context.halo_vid:
                        dut_class = ESYSHaloDut
                        dut_name = 'Halo'
                        app_path = 'sr_app'
                    else:
                        raise ValueError("Detected port USB vid " + str(vid) + " is undefined in environment.py, port auto detection failed")
                    ...
                    app_inst = app_class(app_path, app_config_name, app_target, context)
                    dut.app = app_inst
                    logging.info(dut_name + " is connected to " + port)

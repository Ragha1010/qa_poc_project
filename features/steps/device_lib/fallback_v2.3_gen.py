...
if __name__ == '__main__':
    """Generates Fallback V2.3 result JSON"""
    ...
    while i < int(args.nr_of_cards):
        ...
        while k < number_of_schedules:
            ...
            relay_array_hex[1] = hex((relay_array_hex2 & (0xFF << (0 * 8))) >> (0 * 8))
            relay_array_hex[2] = hex((relay_array_hex2 & (0xFF << (1 * 8))) >> (1 * 8))
            relay_array_hex[3] = hex((relay_array_hex2 & (0xFF << (2 * 8))) >> (2 * 8))
            relay_array_hex[4] = hex((relay_array_hex2 & (0xFF << (3 * 8))) >> (3 * 8))
            relay_array_hex[5] = hex((relay_array_hex2 & (0xFF << (4 * 8))) >> (4 * 8))
            relay_array_hex[6] = hex((relay_array_hex2 & (0xFF << (5 * 8))) >> (5 * 8))
            relay_array_hex[7] = hex((relay_array_hex2 & (0xFF << (6 * 8))) >> (6 * 8))
            relay_array_hex[8] = hex((relay_array_hex2 & (0xFF << (7 * 8))) >> (7 * 8))
